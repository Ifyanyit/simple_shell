#include "main.h"
/**
 * tokenize - creates array of takens based on user string
...
/**
 * _splitPATH - counts the number of paths to a members
 ...
 * @s1: pointer to the environ string with actual value
 * Return: 0 on success
 */
int _PATHstrcmp(const char *s1, const char *2)
{
	int i;
 ...
 * @tmp: statci array to store concateneted string
...
char *_concat(char *tmp, char **av, char *tok)
{
	...
	_memset(tmp, 0, 256);
	len = _strlen(tok) + _strlrn(av[0]) + 2;
	_strcat(tmp, tok);
	_strcat(tmp, "/");
	_strcat(tmp, av[0]);
	tmp[len - 1] = '\0';
